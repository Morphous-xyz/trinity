...
test("executeFlashloan should be encoded and decoded correctly", () => {
	...
	const calldata: any = Trinity.executeFlashloan(
		_tokens,
		_amount,
		_data,
		_isAave,
...
test("multicallFlashloan should be encoded and decoded correctly", () => {
	...
	const calldata: any = Trinity.multicallFlashloan(
		_proxy,
		_deadline,
		_calls,
		_argPos,
...
test("supply should be encoded and decoded correctly", () => {
	...
	const calldataWithMaxGas: any = Trinity.supply(
		_market,
		_poolToken,
		_onBehalf,
		_amount,
	...
	const decodedWithMaxGas =
		TrinityDecoder.decodeSupplyWithMaxGas(calldataWithMaxGas);
...
test("supply (AaveV3) should be encoded and decoded correctly", () => {
	...
	const calldata: any = Trinity.supplyAaveV3(
		_underlying,
		_amount,
		_onBehalf,
		_maxIterations,
...
test("supplyCollateral (AaveV3) should be encoded and decoded correctly", () => {
	...
	const calldata: any = Trinity.supplyCollateralAaveV3(
		_underlying,
		_amount,
		_onBehalf,
	);
...
test("withdraw (AaveV3) should be encoded and decoded correctly", () => {
	...
	const calldata: any = Trinity.withdrawAaveV3(
		_underlying,
		_amount,
		_onBehalf,
		_receiver,
...
test("withdrawCollateral (AaveV3) should be encoded and decoded correctly", () => {
	...
	const calldata: any = Trinity.withdrawCollateralAaveV3(
		_underlying,
		_amount,
		_onBehalf,
		_receiver,
...
test("borrow should be encoded and decoded correctly", () => {
	...
	const calldataWithMaxGas: any = Trinity.borrow(
		_market,
		_poolToken,
		_amount,
		_maxGas,
	...
	const decodedWithMaxGas =
		TrinityDecoder.decodeBorrowWithMaxGas(calldataWithMaxGas);
...
test("borrow with receiver should be encoded and decoded correctly", () => {
	...
	const calldata: any = Trinity.borrowAaveV3(
		_underlying,
		_amount,
		_onBehalf,
		_receiver,
...
test("claim morpho tokens should be encoded and decoded correctly", () => {
	...
	const _proof = [
		toHex("testProofA", { size: 32 }),
		toHex("testProofB", { size: 32 }),
	];
...
test("claim rewards should be encoded and decoded correctly", () => {
	...
	const calldata: any = Trinity.claimRewards(
		_market,
		_poolTokens,
...
test("exchange should be encoded and decoded correctly", () => {
	...
	const calldata: any = Trinity.exchange(
		_aggregator,
		_srcToken,
		_destToken,
		_underlyingAmount,
